...
const SETORES_CONFIG = {
  ...
  outros:      { id: "outros",      label: "📦",          cor1: "#1A1A1A", cor2: "#4D4D4D", cor3: "#808080", cor4: "#B3B3B3" },
}

const getSetor = (nome) => mapaSetor[nome] || "outros"

const FILTROS_RARIDADE = ['todas', 'comum', 'raro', 'epico', 'lendario']
...
// =============================================
// CÉLULA DO GRID — usa CardMinimal
// =============================================
const CartaCell = memo(({ columnIndex, rowIndex, style, cartas, columnCount, onExpand }) => {
...
export default function InventoryScreen() {
  ...
  const [cartaExpandida, setCartaExpandida] = useState(null)  // 🔑 fullscreen
  ...
  const rowHeight = columnWidth > 0
    ? Math.ceil((columnWidth * 4) / 3) + GRID_GAP
    : 0
  ...
  const cartasFiltradas = useMemo(() => {
    ...
    if (filtroRaridade !== 'todas') filtradas = filtradas.filter(c => c.raridade === filtroRaridade)
    if (filtroSetor !== 'todos') filtradas = filtradas.filter(c => c.setor === filtroSetor)
    ...
    filtradas.sort((a, b) => {
      if (ordemSetor[a.setor] !== ordemSetor[b.setor]) return ordemSetor[a.setor] - ordemSetor[b.setor]
      if (ordemRank[a.rank] !== ordemRank[b.rank]) return ordemRank[a.rank] - ordemRank[b.rank]
      return a.nome.localeCompare(b.nome)
    })
    return filtradas
  }, [cartasProcessadas, filtroRaridade, filtroSetor])

  const rowCount = Math.ceil(cartasFiltradas.length / COLUMN_COUNT)

  const handleSetorFilter = useCallback((s) => setFiltroSetor(s), [])
  const handleRaridadeFilter = useCallback((r) => setFiltroRaridade(r), [])

  // Config da carta expandida
